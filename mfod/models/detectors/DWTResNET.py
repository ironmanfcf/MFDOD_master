...
class DWTTransformerattentionlayer(nn.Module):
    ...
    def __init__(self, channels, height, width):
        ...
        self.trans1 = CBAM(in_channels=channels)
        self.trans2 = CBAM(in_channels=channels)
        self.wavelet = WaveletTransform(wavelet='haar')

    def forward(self, x):
        yl, yh = self.wavelet.dwt2_transform(x)
        ...
        LL, HH = self.trans1(LL, HH)

        LH, HL = self.trans2(LH, HL)

        yl = LL
        # 将修改后的LH HL HH重新放入yh中
        new_yh = torch.cat([LH.unsqueeze(2),
                            HL.unsqueeze(2),
                            HH.unsqueeze(2)], dim=2)
        yh[0] = new_yh

        reconstructed_x = self.wavelet.idwt2_transform(yl, yh)
...
class DWTResNetBAM_Channel(BaseModule):
    """ResNet backbone.
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(DWTResNetBAM_Channel, self).__init__(init_cfg)
        ...
        self.att0 = DWTTransformerattentionlayer(256, 128, 128)
        self.att1 = DWTTransformerattentionlayer(512, 64, 64)
        self.att2 = DWTTransformerattentionlayer(1024, 32, 32)
        self.att3 = DWTTransformerattentionlayer(2048, 16, 16)
    ...
    def train(self, mode=True):
        ...
        super(DWTResNetBAM_Channel, self).train(mode)
        self._freeze_stages()
